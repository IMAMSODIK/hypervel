...
<head>
    ...
    <style>
        ...
        @keyframes pulseShadow {
            ...
            0%,
            100% {
                box-shadow: 0 10px 25px rgba(38, 60, 146, 0.18);
            }

            50% {
                box-shadow: 0 18px 40px rgba(38, 60, 146, 0.32);
            }
        }
        ...
        .about-images-wrap .about-badge {
            opacity: 0;
        }

        .about-images-wrap.in-view .about-gold-block {
            animation: fadeInScale 0.7s ease forwards;
        }

        .about-images-wrap.in-view .about-img-main {
            animation: fadeInLeft 0.8s ease 0.15s forwards, floatY 6s ease-in-out 0.95s infinite;
        }

        .about-images-wrap.in-view .about-img-overlay {
            animation: fadeInRight 0.8s ease 0.35s forwards, floatY2 6s ease-in-out 1.75s infinite;
        }

        .about-images-wrap.in-view .about-badge {
            animation: fadeInUp 0.7s ease 0.55s forwards, pulseShadow 3s ease-in-out 1.25s infinite;
        }
        ...
        .slider-dot {
            ...
            background: rgba(38, 60, 146, 0.22);
            transition: all .25s;
            cursor: pointer;
        }
...
<body class="bg-white text-slate-800">
    ...
    @php
        ...
        $heroSubtitle = \App\Models\Setting::get(
            'hero_subtitle',
            'Your trusted partner for industrial valve distribution and after-sales service.',
        );
    ...
    <nav class="bg-white shadow-md sticky top-0 z-40">
        ...
        <div class="md:hidden hidden border-t border-slate-100" id="mobileMenu">
            <div class="flex flex-col gap-2 p-4 text-sm font-medium text-slate-600">
                ...
                <a href="#contact" class="bg-primary text-white px-4 py-2 rounded-lg text-center font-semibold">Get a
                    Quote</a>
            </div>
        </div>
    </nav>
        ...
        <div class="relative z-10 h-full flex items-center">
            <div class="max-w-7xl mx-auto px-4 w-full">
                <div class="max-w-2xl text-white">
                    <span
                        class="inline-block bg-gold text-primary text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-4">Industrial
                        Valve &amp; Actuator</span>
                    <h1 class="text-3xl md:text-5xl font-extrabold leading-tight mb-4 drop-shadow-lg">
                        {{ $heroTitle }}</h1>
            ...
            <div class="max-w-7xl mx-auto px-4">
                <div class="flex flex-wrap justify-center gap-6 text-center mx-auto" style="max-width:912px;">
                    @foreach ($statistics as $stat)
                        <div class="group relative bg-white rounded-xl px-4 py-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg w-full sm:w-auto sm:flex-1"
                            style="min-width:180px;max-width:208px;border:1px solid rgba(0,0,0,0.06);">
                            <div
                                class="absolute top-0 left-0 right-0 h-0.5 rounded-t-xl bg-gradient-to-r from-gold to-gold-light opacity-0 group-hover:opacity-100 transition-opacity">
                            </div>
                            @if ($stat->icon)
                                <div
                                    class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary/10 mb-3 transition-transform duration-300 group-hover:scale-110">
                                    <i class="bi {{ $stat->icon }} text-primary text-lg"></i>
                                </div>
                            @endif
                            <div class="text-2xl font-extrabold mb-1 text-primary">
                                <span class="stat-counter"
                                    data-target="{{ $stat->value }}">0</span>{{ $stat->suffix }}
                            </div>
            ...
            <div class="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div class="flex flex-col gap-3 mb-8">
                            @foreach ($aboutBullets as $bullet)
                                <div class="flex items-center gap-3 text-slate-700">
                                    <div
                                        class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-primary/10 shrink-0">
                ...
                <div class="relative about-images-wrap" style="height:460px;">
                    ...
                    <div class="absolute pointer-events-none"
                        style="bottom:6px;right:6px;width:60%;height:68%;background-image:radial-gradient(rgba(38,60,146,0.14) 1.5px,transparent 1.5px);background-size:18px 18px;z-index:0;">
                    </div>
                    {{-- Gold accent block peeking top-left --}}
                    <div class="absolute rounded-2xl bg-gold about-gold-block"
                        style="width:70%;height:78%;top:-14px;left:-14px;z-index:1;"></div>
                    {{-- Main image --}}
                    <div class="absolute rounded-2xl overflow-hidden shadow-xl ring-1 ring-black/5 z-10 about-img-main"
                        style="width:70%;height:78%;top:0;left:0;">
                        @if ($aboutImage1)
                            <img src="{{ $aboutImage1 }}"
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center text-white/70"
                                style="background:linear-gradient(135deg,#263C92,#1d2d6e);"><i
                                    class="bi bi-image text-5xl"></i></div>
                        @endif
                    </div>
                    {{-- Overlay image (front) --}}
                    @if ($aboutImage2)
                        <div class="absolute rounded-2xl overflow-hidden shadow-2xl ring-4 ring-white z-20 about-img-overlay"
                            style="width:48%;height:62%;bottom:0;right:0;">
                            <img src="{{ $aboutImage2 }}"
                    ...
                    @if ($expStat)
                        <div class="absolute bg-primary text-white rounded-2xl shadow-xl px-5 py-3 text-center z-30 about-badge"
                            style="bottom:-16px;left:6%;">
                            <div class="text-3xl font-extrabold text-gold leading-none">
                                {{ $expStat->value }}{{ $expStat->suffix }}</div>
    ...
    <section id="products" class="bg-slate-50 py-20">
        <div class="max-w-7xl mx-auto px-4">
            <div class="text-center mb-12">
                ...
                <p class="text-slate-500 mt-4 max-w-2xl mx-auto">Discover our comprehensive range of industrial
                    equipment, engineered for reliability and performance.</p>
            </div>
            @if ($products->isNotEmpty())
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach ($products as $product)
                        <a href="{{ route('products.show', $product) }}"
                            class="group bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
                            <div class="h-52 relative overflow-hidden">
                                @if ($product->featured_image)
                                    <img src="{{ $product->featured_image_url }}" alt="{{ $product->title }}"
                                        class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                    <div
                                        class="absolute inset-0 bg-gradient-to-t from-primary-dark/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                                    </div>
                                @else
                                    <div class="w-full h-full flex items-center justify-center"
                                        ...
                                        @if ($product->icon)
                                            <i
                                                class="bi {{ $product->icon }} text-white/90 text-7xl group-hover:scale-110 transition-transform duration-500"></i>
                                        @else
                                ...
                                @endif
                                <span
                                    class="absolute top-4 left-4 inline-flex items-center justify-center w-9 h-9 rounded-full bg-gold text-primary font-bold text-sm shadow-md">{{ $loop->index + 1 }}</span>
                            </div>
                            <div class="p-6 flex-grow flex flex-col">
                                <h3
                                    class="font-bold text-lg text-slate-800 mb-2 group-hover:text-primary transition-colors">
                                    {{ $product->title }}</h3>
                                <div
                                    class="w-10 h-0.5 bg-gold rounded-full mb-3 transition-all duration-300 group-hover:w-16">
                                </div>
                                <p class="text-sm text-slate-500 leading-relaxed flex-grow">
                                    {{ $product->short_description }}</p>
                                <span
                                    class="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary group-hover:gap-2.5 transition-all">
    ...
    <section id="services" class="py-20 relative overflow-hidden"
        style="background:linear-gradient(180deg,#f8fafc 0%,#eef2ff 60%,#f8fafc 100%);">
        {{-- Decorative blobs --}}
        ...
        <div class="max-w-7xl mx-auto px-4 relative z-10">
            <div class="text-center mb-12">
                ...
                <div class="w-20 h-1 bg-gold mx-auto mt-4 rounded-full"></div>
                <p class="text-slate-500 mt-4 max-w-2xl mx-auto">Comprehensive engineering support across the
                    industrial value chain — from consultancy to installation, spares, and training.</p>
            </div>
            ...
            @if ($services->isNotEmpty())
                ...
                <div class="services-slider relative">
                    ...
                    <div class="services-track-wrap overflow-hidden px-1">
                        <div class="services-track flex transition-transform duration-500 ease-out gap-5"
                            id="servicesTrack">
                            @foreach ($services as $service)
                                <div class="services-slide shrink-0">
                                    <article
                                        class="group relative bg-white rounded-2xl overflow-hidden shadow-sm ring-1 ring-slate-100 hover:ring-primary/20 transition-all duration-300 h-full flex flex-col">
                                        {{-- Image / icon header --}}
                                        <div class="relative h-44 overflow-hidden">
                                            @if ($service->image)
                                                <img src="{{ $service->image_url }}" alt="{{ $service->title }}"
                                                    class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                                <div
                                                    class="absolute inset-0 bg-gradient-to-t from-primary-dark/70 via-primary-dark/20 to-transparent">
                                                </div>
                                            @else
                                                <div class="w-full h-full flex items-center justify-center"
                                                    style="background:linear-gradient(135deg,#263C92,#1d2d6e);">
                                                    @if ($service->icon)
                                                        <i
                                                            class="bi {{ $service->icon }} text-white/90 text-6xl group-hover:scale-110 transition-transform duration-500"></i>
                                                    @else
                                            ...
                                            {{-- Number badge --}}
                                            <span
                                                class="absolute top-4 left-4 inline-flex items-center justify-center w-8 h-8 rounded-full bg-gold text-primary font-bold text-sm shadow-md">{{ $loop->index + 1 }}</span>
                                            @if ($service->icon && $service->image)
                                                <span
                                                    class="absolute bottom-4 left-4 inline-flex items-center justify-center w-11 h-11 rounded-xl bg-white/95 shadow-md">
                                        ...
                                        <div class="p-6 flex flex-col flex-grow">
                                            ...
                                            <a href="#contact"
                                                class="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:gap-2.5 transition-all">
                    ...
                    {{-- Controls --}}
                    <button
                        class="slider-btn slider-prev absolute top-1/2 -translate-y-1/2 -left-2 md:-left-5 z-20 inline-flex items-center justify-center w-11 h-11 rounded-full bg-white shadow-lg ring-1 ring-slate-200 text-primary hover:bg-primary hover:text-white transition-all duration-300 hover:scale-110 disabled:opacity-40 disabled:hover:scale-100 disabled:hover:bg-white disabled:hover:text-primary"
                        aria-label="Previous">
                        <i class="bi bi-chevron-left text-xl"></i>
                    </button>
                    <button
                        class="slider-btn slider-next absolute top-1/2 -translate-y-1/2 -right-2 md:-right-5 z-20 inline-flex items-center justify-center w-11 h-11 rounded-full bg-white shadow-lg ring-1 ring-slate-200 text-primary hover:bg-primary hover:text-white transition-all duration-300 hover:scale-110 disabled:opacity-40 disabled:hover:scale-100 disabled:hover:bg-white disabled:hover:text-primary"
                        aria-label="Next">
        ...
        <div class="max-w-7xl mx-auto px-4 relative z-10">
            ...
            @if ($featuredProjects->isNotEmpty())
                            ...
                            <div class="h-56 relative overflow-hidden">
                                ...
                                @else
                                    <div class="w-full h-full flex items-center justify-center"
                                        ...
                                        @if ($project->icon)
                                            <i
                                                class="bi {{ $project->icon }} text-white/90 text-7xl group-hover:scale-110 transition-transform duration-500"></i>
                                        @else
                                ...
                                @if ($project->category)
                                    <span
                                        class="absolute top-4 left-4 bg-gold text-primary text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-md">{{ $project->category }}</span>
                                @endif
                                @if ($project->year)
                                    <span
                                        class="absolute bottom-4 right-4 bg-white/95 text-slate-700 text-xs font-bold px-2.5 py-1 rounded-full shadow-md">{{ $project->year }}</span>
                                @endif
                            </div>
                            <div class="p-6 flex-grow flex flex-col">
                                <h3
                                ...
                                @if ($project->client || $project->location)
                                    <div
                                        class="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                                        @if ($project->client)
                                            <span class="inline-flex items-center gap-1"><i
                                                    class="bi bi-building"></i>{{ $project->client }}</span>
                                        @endif
                                        @if ($project->location)
                                            <span class="inline-flex items-center gap-1"><i
                                                    class="bi bi-geo-alt"></i>{{ $project->location }}</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
                ...
                <div class="text-center mt-10">
                    <a href="{{ route('projects.index') }}"
                        class="inline-flex items-center gap-2 bg-primary text-white font-semibold px-7 py-3 rounded-lg hover:bg-primary-dark transition-colors">
                        View More Projects <i class="bi bi-arrow-right"></i>
                    </a>
                </div>
            @else
            ...
            <div class="max-w-7xl mx-auto px-4">
                ...
                <div class="flex flex-wrap justify-center gap-6">
                    @foreach ($clients as $client)
                        <div class="group relative bg-white rounded-xl border border-slate-100 shadow-sm flex items-center justify-center overflow-hidden transition-all duration-300 hover:shadow-md hover:border-primary/20"
                            style="aspect-ratio:3/2;width:calc((100% - 6rem)/5);max-width:200px;min-width:130px;">
                            @if ($client->logo)
                                <img src="{{ $client->logo_url }}" alt="{{ $client->name }}"
                                    class="max-w-full max-h-full object-contain p-4 transition-all duration-300 {{ $client->website ? 'group-hover:opacity-0' : '' }}">
                            @else
                                <div
                                    class="flex items-center justify-center w-12 h-12 rounded-lg bg-primary text-white">
                                    <i class="bi bi-building text-xl"></i>
                                </div>
                            @endif
                            {{-- Hover: company name overlay --}}
                            <div
                                class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 {{ $client->website ? 'bg-primary/90' : 'bg-primary/85' }}">
                                <div class="text-center px-3">
                                    <div class="text-white font-bold text-sm leading-tight">{{ $client->name }}</div>
                                    @if ($client->website)
                                        <div class="text-gold text-xs mt-1">Visit site <i
                                                class="bi bi-box-arrow-up-right"></i></div>
                                    @endif
                                </div>
                            </div>
                            @if ($client->website)
                                <a href="{{ $client->website }}" target="_blank" class="absolute inset-0 z-10"
                                    aria-label="{{ $client->name }}"></a>
                            @endif
                        </div>
                    @endforeach
                </div>
    ...
    <section id="contact" class="bg-primary py-20 text-white">
        <div class="max-w-7xl mx-auto px-4">
            <div class="text-center mb-12">
                ...
                <div class="w-20 h-1 bg-gold mx-auto mt-4 rounded-full"></div>
                <p class="text-slate-200 mt-4 max-w-2xl mx-auto">Have a question or want to collaborate? Send us a
                    message and our team will get back to you.</p>
            </div>
            ...
            <div class="grid lg:grid-cols-2 gap-10 items-start">
                    ...
                    <div class="grid sm:grid-cols-2 gap-6 mb-8">
                        @if ($phone)
                            <div class="p-6 bg-white/10 rounded-xl">
                                <div
                                    class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gold text-primary mb-4">
                                    <i class="bi bi-telephone-fill text-2xl"></i></div>
                                <h3 class="font-bold mb-2">Phone</h3>
                                @if ($phone2)
                                    <p class="text-sm text-slate-200">{{ $phone }}</p>
                                <p class="text-sm text-slate-200">{{ $phone2 }}</p>@else<p
                                        class="text-sm text-slate-200">{{ $phone }}</p>
                                @endif
                            </div>
                        @endif
                        @if ($email)
                            <div class="p-6 bg-white/10 rounded-xl">
                                <div
                                    class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gold text-primary mb-4">
                                    <i class="bi bi-envelope-fill text-2xl"></i></div>
                        ...
                        @if ($address)
                            <div class="p-6 bg-white/10 rounded-xl">
                                <div
                                    class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gold text-primary mb-4">
                                    <i class="bi bi-geo-alt-fill text-2xl"></i></div>
                        ...
                        @if ($hours)
                            <div class="p-6 bg-white/10 rounded-xl">
                                <div
                                    class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gold text-primary mb-4">
                                    <i class="bi bi-clock-fill text-2xl"></i></div>
                ...
                <div class="bg-white rounded-2xl shadow-2xl p-8">
                    @if (session('inquiry_success'))
                        <div
                            class="mb-4 p-3 rounded-lg bg-green-50 border border-green-200 text-green-700 text-sm flex items-start gap-2">
                    ...
                    <form method="POST" action="{{ route('contact.inquiry') }}" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Full Name <span
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Message <span
                                    class="text-red-500">*</span></label>
                        ...
                        <button type="submit"
                            class="w-full bg-primary text-white font-semibold px-6 py-3 rounded-lg hover:bg-primary-dark transition-colors inline-flex items-center justify-center gap-2">
    ...
    <script>
        ...
        (function() {
            ...
            function buildClones() {
                // clear old clones
                Array.prototype.forEach.call(track.querySelectorAll('.clone-slide'), function(el) {
                    el.remove();
                });

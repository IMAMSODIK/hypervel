...
<nav class="bg-white shadow-md sticky top-0 z-40">
    <div class="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <div class="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
            ...
            <a href="{{ url('/') }}#products" class="text-primary font-semibold">Products</a>
    ...
    <div class="md:hidden hidden border-t border-slate-100" id="mobileMenu">
        <div class="flex flex-col gap-2 p-4 text-sm font-medium text-slate-600">
            ...
            <a href="{{ url('/') }}#products" class="py-1 text-primary font-semibold">Products</a>
...
<section class="relative w-full overflow-hidden" style="height:60vh; min-height:420px;">
    ...
    <div class="relative z-10 h-full flex items-center justify-center">
        <div class="max-w-3xl mx-auto px-4 text-center text-white">
            <span class="inline-block bg-gold text-primary text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-4">Product</span>
            <h1 class="text-3xl md:text-5xl font-extrabold leading-tight mb-4 drop-shadow-lg">{{ $product->title }}</h1>
            @if($product->short_description)
                <p class="text-lg text-slate-100 mb-8 leading-relaxed drop-shadow max-w-2xl mx-auto">{{ $product->short_description }}</p>
            @endif
            <div class="flex flex-wrap gap-4 justify-center">
                <a href="#product-detail" class="bg-gold text-primary font-semibold px-6 py-3 rounded-lg hover:opacity-90 transition-opacity inline-flex items-center gap-2">
                ...
                <a href="{{ url('/') }}#contact" class="border border-white/40 text-white font-semibold px-6 py-3 rounded-lg hover:bg-white/10 transition-colors inline-flex items-center gap-2 backdrop-blur-sm">
                    <i class="bi bi-telephone"></i> Get a Quote
                </a>
            </div>
        </div>
    </div>
...
{{-- Product Detail --}}
<section id="product-detail" class="py-12">
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div class="lg:sticky lg:top-24 lg:self-start">
                @php
                    $mainImage = $product->featured_image
                        ...
                        : ($product->images->isNotEmpty() ? $product->images->first()->image_url : null);
                @endphp
                <div class="relative rounded-2xl overflow-hidden shadow-xl bg-slate-100" style="aspect-ratio:4/3;">
                    @if($mainImage)
                        <img src="{{ $mainImage }}" alt="{{ $product->title }}" id="mainProductImage" class="w-full h-full object-cover transition-all duration-300">
                    @else
                        <div class="w-full h-full flex items-center justify-center" style="background:linear-gradient(135deg,#263C92,#1d2d6e);">
                            @if($product->icon)
                                <i class="bi {{ $product->icon }} text-white/90 text-8xl"></i>
                            @else
                ...
                @if($product->images->count() > 0 || $product->featured_image)
                    <div class="flex gap-3 mt-4 overflow-x-auto pb-2" id="galleryThumbs">
                        @if($product->featured_image)
                            <button class="gallery-thumb active shrink-0 rounded-lg overflow-hidden border-2 border-transparent" data-src="{{ $product->featured_image_url }}">
                                <img src="{{ $product->featured_image_url }}" alt="Featured" class="w-20 h-20 object-cover">
                            </button>
                        @endif
                        @foreach($product->images as $img)
                            <button class="gallery-thumb shrink-0 rounded-lg overflow-hidden border-2 border-transparent @if(!$product->featured_image && $loop->first) active @endif" data-src="{{ $img->image_url }}">
            ...
            <div class="animate-fade-up">
                <h2 class="text-2xl md:text-3xl font-bold text-slate-800 mb-3">Overview</h2>
                <div class="w-16 h-1 bg-gold rounded-full mb-6"></div>

                @if($product->short_description)
                    <p class="text-lg text-slate-600 leading-relaxed mb-6">{{ $product->short_description }}</p>
                @endif
                ...
                {{-- Rich text content --}}
                @if($product->content)
                    <div class="rich-content border-t border-slate-100 pt-6">{!! $product->content !!}</div>
                @endif
            </div>
        </div>
    </div>
...
<section class="bg-slate-50 py-16">
    <div class="max-w-7xl mx-auto px-4">
        <div class="mb-10">
            ...
            <h2 class="text-2xl md:text-3xl font-bold text-slate-800 mt-1">Related Products</h2>
            <div class="w-16 h-1 bg-gold rounded-full mt-3"></div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($related as $rel)
                <a href="{{ route('products.show', $rel) }}" class="group bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <div class="h-40 overflow-hidden relative">
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center" style="background:linear-gradient(135deg,#263C92,#1d2d6e);">
                                <i class="bi {{ $rel->icon ?: 'bi-grid' }} text-white/90 text-5xl"></i>
                            </div>
                        @endif
                    </div>
                    <div class="p-5">

...
<head>
    ...
    @if($project->meta_description)<meta name="description" content="{{ $project->meta_description }}">@endif
    @if($project->meta_keywords)<meta name="keywords" content="{{ $project->meta_keywords }}">@endif
    ...
    <style>
        ...
        .rich-content { line-height: 1.8; color: #334155; }
        .rich-content h1 { font-size: 1.875rem; font-weight: 700; color: #1e293b; margin: 1.5rem 0 0.75rem; }
        .rich-content h2 { font-size: 1.5rem; font-weight: 700; color: #1e293b; margin: 1.5rem 0 0.75rem; }
        .rich-content h3 { font-size: 1.25rem; font-weight: 600; color: #1e293b; margin: 1.25rem 0 0.5rem; }
        .rich-content h4 { font-size: 1.125rem; font-weight: 600; color: #1e293b; margin: 1rem 0 0.5rem; }
        ...
        .rich-content blockquote { border-left: 4px solid #DFB624; padding: 0.5rem 1rem; margin: 1rem 0; background: #f8fafc; color: #475569; }
        ...
        .rich-content pre { background: #1e293b; color: #e2e8f0; padding: 1rem; border-radius: 0.5rem; overflow-x: auto; margin: 1rem 0; }

        .gallery-thumb { cursor: pointer; transition: all .25s; }
        .gallery-thumb.active { ring: 2px solid #DFB624; box-shadow: 0 0 0 2px #DFB624; }
...
<section class="relative w-full overflow-hidden" style="height:60vh; min-height:420px;">
    ...
    <div class="relative z-10 h-full flex items-center justify-center">
        <div class="max-w-3xl mx-auto px-4 text-center text-white">
            <span class="inline-block bg-gold text-primary text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-4">Project</span>
            <h1 class="text-3xl md:text-5xl font-extrabold leading-tight mb-4 drop-shadow-lg">{{ $project->title }}</h1>
            @if($project->short_description)
                <p class="text-lg text-slate-100 mb-8 leading-relaxed drop-shadow max-w-2xl mx-auto">{{ $project->short_description }}</p>
            @endif
            <div class="flex flex-wrap gap-4 justify-center">
                <a href="#project-detail" class="bg-gold text-primary font-semibold px-6 py-3 rounded-lg hover:opacity-90 transition-opacity inline-flex items-center gap-2">
                    View Details <i class="bi bi-arrow-down"></i>
                </a>
                <a href="{{ route('projects.index') }}" class="border border-white/40 text-white font-semibold px-6 py-3 rounded-lg hover:bg-white/10 transition-colors inline-flex items-center gap-2 backdrop-blur-sm">
    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div class="lg:sticky lg:top-24 lg:self-start">
                @php
                    $mainImage = $project->featured_image
                        ...
                        : ($project->images->isNotEmpty() ? $project->images->first()->image_url : null);
                @endphp
                <div class="relative rounded-2xl overflow-hidden shadow-xl bg-slate-100" style="aspect-ratio:4/3;">
                    @if($mainImage)
                        <img src="{{ $mainImage }}" alt="{{ $project->title }}" id="mainProjectImage" class="w-full h-full object-cover transition-all duration-300">
                    @else
                ...
                @if($project->images->count() > 0 || $project->featured_image)
                    <div class="flex gap-3 mt-4 overflow-x-auto pb-2" id="galleryThumbs">
                        @if($project->featured_image)
                            <button class="gallery-thumb active shrink-0 rounded-lg overflow-hidden border-2 border-transparent" data-src="{{ $project->featured_image_url }}">
                                <img src="{{ $project->featured_image_url }}" alt="Featured" class="w-20 h-20 object-cover">
                            </button>
                        @endif
                        @foreach($project->images as $img)
                            <button class="gallery-thumb shrink-0 rounded-lg overflow-hidden border-2 border-transparent @if(!$project->featured_image && $loop->first) active @endif" data-src="{{ $img->image_url }}">
            ...
            <div class="animate-fade-up">
                ...
                <div class="grid grid-cols-2 gap-3 mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                    @if($project->category)
                        <div>
                            <div class="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-0.5">Category</div>
                            <div class="text-sm font-medium text-slate-700">{{ $project->category }}</div>
                        </div>
                    @endif
                    @if($project->client)
                        <div>
                            <div class="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-0.5">Client</div>
                            <div class="text-sm font-medium text-slate-700">{{ $project->client }}</div>
                        </div>
                    @endif
                    @if($project->location)
                        <div>
                            <div class="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-0.5">Location</div>
                            <div class="text-sm font-medium text-slate-700">{{ $project->location }}</div>
                        </div>
                    @endif
                    @if($project->year)
                        <div>
                            <div class="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-0.5">Year</div>
                ...
                <div class="flex flex-wrap gap-3 mb-8">
                    ...
                    <a href="{{ url('/') }}#contact" class="inline-flex items-center gap-2 border border-slate-200 text-slate-700 font-semibold px-6 py-3 rounded-lg hover:bg-slate-50 transition-colors">
    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($related as $rel)
                <a href="{{ route('projects.show', $rel->slug) }}" class="group bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <div class="h-40 overflow-hidden relative">
                        @if($rel->featured_image)
                            <img src="{{ $rel->featured_image_url }}" alt="{{ $rel->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                        @else
                            <div class="w-full h-full flex items-center justify-center" style="background:linear-gradient(135deg,#263C92,#1d2d6e);">
                    ...
                    <div class="p-5">
                        <h3 class="font-bold text-slate-800 group-hover:text-primary transition-colors">{{ $rel->title }}</h3>
                        <p class="text-sm text-slate-500 mt-1">{{ $rel->short_description }}</p>
                        <span class="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-primary group-hover:gap-2.5 transition-all">
    ...
    <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-3 gap-8">
        ...
        <div>
            ...
            <ul class="space-y-2 text-sm">
                ...
                <li><a href="{{ url('/') }}#projects" class="hover:text-gold transition-colors">Projects</a></li>
